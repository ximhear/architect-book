...
> 같은 트레이드오프 박스가 26명 조직과 220명 조직에서 다른 결정으로 이어진다.

## 이 장에서 답하는 질문

- 26명 조직의 아키텍트는 매일 무엇을 하는가?
- 1권의 NFR 7가지가 동네에너지에서는 어떻게 우선순위가 달라지는가?
- 모놀리스 / 모듈러 / MSA — 26명 조직에서는 어떻게 결정하는가?
- 공공 협력은 의사결정에 어떤 새로운 제약을 추가하는가?

> 약어 풀이 — NFR (Non-Functional Requirement, 비기능 요구사항), MSA (Microservices Architecture), ADR (Architecture Decision Record), Bounded Context (DDD 의 도메인 경계), DR (Demand Response, 수요반응), SRE (Site Reliability Engineering), IoT (Internet of Things), IDP (Internal Developer Platform).

## 들어가며

[1권 1장](../../manuscript/01-아키텍처-기초/README.md) 에서 다룬 사고 골격은 그대로다 — 비기능 요구사항(NFR), Bounded Context, ADR, C4 모델.
달라지는 것은 **NFR 우선순위 · 스타일 선택 · 조직 자율성** 이 26명·공공 협력의 무게 아래에서 어떻게 다른 결정을 만드는가다.
...
### 1.2 흔한 함정 — 26명에서 220명 책을 그대로 따라가지 말 것

> - **MSA 80개** — 220명에서도 빠듯한데 26명에서는 운영 불가능. 모놀리스 + 1~3개 추출이 한계
> - **SRE 별도 팀** — 26명에서 SRE 3명은 한 본부. 인프라/플랫폼 4명이 SRE·DevOps·관측성·보안 겸함
> - **도메인팀 분리** — 26명에서 도메인을 7개로 나누면 팀당 평균 2명. 인지 부하 부족 → 모놀리스 안에서 모듈 경계
> - **자체 관측성 self-host** — Loki / Tempo / Prometheus 운영은 SRE 1.5명. 매니지드 (NCP Cloud Insight) 가 거의 강제

## 2. 비기능 요구사항(NFR) — 동네에너지의 우선순위 재배치

1권의 NFR 7가지 (성능·가용성·확장성·보안·운영성·유지보수성·비용) 는 그대로. **우선순위가 다르다**.

> **출처** — 1권 우선 칸은 [1권 1장 2절](../../manuscript/01-아키텍처-기초/README.md#2-비기능-요구사항-nfr) 인용. 2권 우선 칸은 [케이스 6 운영 SLA](../case-study/README.md#6-운영-sla) + [케이스 10.1 구체 신호](../case-study/README.md#101-구체-신호-수치-기준) 인용. 1권 매출 대비 IT 비용 6~8% 는 1권 케이스의 가정 수치.

| NFR | 1권 (커머스) 우선 | 2권 (동네에너지) 우선 |
|-----|------------------|---------------------|
| 가용성 | ★★★ (결제 99.99%) | ★★★★ (**취약계층 알람 99.99% + IoT 손실률 0.1%**) |
| 성능 | ★★★ (p95 < 200ms) | ★★ (시민 앱 p95 < 500ms — 시니어 고려) |
| 확장성 | ★★★ (10배 트래픽) | ★★ (DR 5,000 TPS 폭주 외엔 평이, 호출 월 5~10회) |
| 보안 | ★★★ (PII + 결제) | ★★★★ (**준 민감정보 + 공공 위탁 1건 사고 = 사업 중단**) |
| 운영성 | ★★ | ★★★★ (**26명 운영 — 자동화 없으면 불가**) |
| 유지보수성 | ★★ | ★★ |
...
| **운영성** | SRE 8 + 도메인팀 1차 | **인프라 4명이 모든 SRE / 도메인팀 = 백엔드 1팀** | 인력 절대 부족 |
...
| 질문 | 1권 (원픽 220명) | 2권 (동네에너지 26명) |
|------|----------------|---------------------|
| 팀이 30명 이상? | Yes | **No** |
| 서로 다른 도메인이 명확히 분리됨? | Yes (커머스·결제·검색 분명) | **부분적** (IoT 와 매칭은 다르지만 그 외는 결합 강) |
| 팀별로 배포 주기가 다름? | Yes | **No** (한 팀 = 한 배포) |
| 운영 인력 충분 (DevOps/SRE)? | Yes | **No** (4명, IoT 운영 / CSAP 협업 겸함 — 1권 대비 1명 증원 후도 부족 신호) |
...
1권에는 없던 영역. 26명 조직의 아키텍트는 외부 협력 시간이 20% 라고 했다.
...
| 클라우드 | AWS 메인 + NCP 보조 | **NCP 단일** (공공 가점 / 데이터 주권) |
| 관측성 | Grafana Cloud (변경 후) | **NCP Cloud Insight + Cloud Log Analytics** (처음부터 — 데이터 주권) |
| 데이터 | 도메인별 분리 (6종) | **한 PostgreSQL + TimescaleDB extension** (운영 단순) |
| 외부 협력 | PG / 본인인증 | **+ 공공 (한전 / KPX / 세종시 / 복지부)** |
| 운영 | SRE 본부 / 도메인팀 자율 | **인프라 4명이 SRE 겸함 / 백엔드팀 1차 온콜** |

### 트레이드오프 — 1권 결정을 그대로 가져왔다면

| 1권 결정 | 2권에서 그대로 채택 시 결과 |
|---------|---------------------------|
| AWS 메인 + NCP 보조 | 공공 사업 가점 손실 + 운영 인력 2배 부담 (멀티 클라우드) + 세종시 데이터허브 호환 부담 |
| 도메인 DB 분리 (6종) | 운영 인력 부족 → 백업·튜닝 누락 → 데이터 사고 |
| SRE 별도 본부 | 26명 중 5명을 SRE 에 — 도메인 개발 멈춤 |

...
| **Anti-Corruption Layer (부패 방지 계층)** | 외부 모델을 내부 모델로 번역 | PG 응답 → 결제 도메인 모델 |
...
| 2 | 1단계 완료 | 검색·추천 별도 서비스, OS·Redis 직접 호출 | 메인 DB CPU 점유 < 30% |
| 3 | 2단계 완료 + 결제 트랜잭션 격리 가능 | 결제 별도 서비스, NCP 100% 이중화 | 결제 SLO 99.99% 분기 1회 달성 |
